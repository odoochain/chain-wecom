...
class WecomUser(models.Model):
    ...
    enable = fields.Boolean(
        string="Enable", readonly=True, default=True
    )  # 启用/禁用成员。1表示启用成员，0表示禁用成员
    isleader = fields.Boolean(
        string="Is leader", readonly=True, default=False
    )  # 上级字段，标识是否为上级。第三方仅通讯录应用可获取
    hide_mobile = fields.Boolean(
        string="Hide mobile", readonly=True, default=False
    )  # 是否隐藏手机号
    ...
    department_leader = fields.Char(
        ...
        default="",
        compute="_compute_department_leader",
        store=True,
    )  # 部门领导 readonly=True,

    direct_leader_id = fields.Many2one(
        "wecom.user",
        "Direct Leader",
        domain="[('company_id', '=', company_id)]",
        compute="_compute_direct_leader_id",
        store=True,
    )  #
    ...
    def copy_as_system_user(self):
        ...
        if contacts_allow_add_system_users:
            ...
            msg = {
                "title": _("Error!"),
                "message": _(
                    "The current configuration does not allow replication as a system user!"
                ),
                "sticky": False,
            }
            ...
            user.create(
                {
                    ...
                    "direct_leader": wecom_user["direct_leader"][0]
                    if len(wecom_user["direct_leader"]) > 0
                    else wecom_user["direct_leader"],
