...
class WecomServerApiError(models.Model):
        ...
        try:
            ...
            for index, error in enumerate(error_results):
                ...
                if error["method"] == "查看帮助":
                    ...
                    error["method"] = self.replaceErrorMethod(str(error["code"]),
                                                              "https://developer.work.weixin.qq.com/document/path/90313"
                                                              "#%E9%94%99%E8%AF%AF%E7%A0%81%EF%BC%9A "
                                                              )
                errors.append(error)
    ...
    def replaceErrorMethod(self, code, methodshtml):
        """
        替换 html 排查方法
        """
        method = str(methodshtml + (str(code)))
